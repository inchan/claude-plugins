...
### Step 5: Self-Evaluation & Improvement Loop (Optional)

복잡한 작업의 경우, 요구사항을 제시하기 전에 자체 평가를 수행합니다:
...
```
생성 → 자체 평가 → 약점 발견 → 개선 → 재평가
         ↑                              ↓
         └──────── (필요 시 반복) ←──────┘
```

**참고**: 이 단계는 복잡한 작업(Extended Template)에서 권장되며, 단순 작업에서는 생략할 수 있습니다.
