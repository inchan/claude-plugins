...
### 4️⃣ 복잡한 작업 자동 분할 (NEW!)

```
👤 사용자: "전체 인증 시스템을 리팩토링해줘 - 로그인, 회원가입, 비밀번호 재설정, OAuth 통합, 세션 관리"

🤖 Claude:
[복잡도 평가]
"이 작업을 분석한 결과:
- 변경 파일: 8개 (HIGH)
- 리팩토링 범위: 전체 아키텍처 (HIGH)
- 로직 복잡도: 다중 인증 플로우 (HIGH)
- 의존성: 여러 모듈 간 상호작용 (HIGH)

복잡도 점수: HIGH (4/4)

이 작업을 서브태스크로 분할하시겠습니까?"

👤 사용자: "네"

🤖 Claude:
"작업을 4개의 서브태스크로 분할했습니다:

Task Decomposition Plan:
[→] Sub-task 1: 인증 데이터 모델 리팩토링 (IN PROGRESS)
[ ] Sub-task 2: 로그인/회원가입 로직 개선 (PENDING - depends on 1)
[ ] Sub-task 3: OAuth 통합 구현 (PENDING - depends on 2)
[ ] Sub-task 4: 세션 관리 및 비밀번호 재설정 (PENDING - depends on 2)

현재 Sub-task 1을 시작합니다..."

[Sub-task 1: 계획 → Codex 검증 → 구현 → Codex 리뷰 → 완료]

🤖 Claude:
"Sub-task 1 완료!

Task Decomposition Plan:
[✓] Sub-task 1: 인증 데이터 모델 리팩토링 (COMPLETED)
[→] Sub-task 2: 로그인/회원가입 로직 개선 (IN PROGRESS)
[ ] Sub-task 3: OAuth 통합 구현 (PENDING)
[ ] Sub-task 4: 세션 관리 및 비밀번호 재설정 (PENDING)

다음 Sub-task 2를 시작합니다..."

[각 서브태스크마다 Plan → Validate → Implement → Review 루프 실행]
...
Task Decomposition Plan:
[✓] Sub-task 1: 인증 데이터 모델 리팩토링 (COMPLETED)
[✓] Sub-task 2: 로그인/회원가입 로직 개선 (COMPLETED)
[✓] Sub-task 3: OAuth 통합 구현 (COMPLETED)
[✓] Sub-task 4: 세션 관리 및 비밀번호 재설정 (COMPLETED)

전체 인증 시스템 리팩토링이 완료되었습니다!"
```
...
- 보안/성능이 critical한 작업
- 리팩토링 대규모 작업
- **여러 파일 동시 수정 (5개 이상)** ⭐ NEW
- **아키텍처 변경이 필요한 작업** ⭐ NEW
- **복잡한 비즈니스 로직 구현** ⭐ NEW

✨ **자동 작업 분할 기능:**
- 스킬이 자동으로 복잡도를 평가합니다
- 복잡한 작업은 서브태스크로 분할 제안
- 각 서브태스크는 독립적으로 Plan → Validate → Implement → Review
- 리뷰 시간이 길어지지 않고 품질 유지
...
### 💡 Tip 5: 작업 분할 (NEW!)
```
복잡도 평가 → 분할 제안 → 사용자 승인 → 서브태스크 실행

복잡도 기준:
- Low (0-2 factors): 정상 진행
- Medium (3-4 factors): 분할 고려
- High (5+ factors): 분할 강력 권장

평가 요소:
- 파일 수 (>5개 = HIGH)
- 변경 범위 (아키텍처 = HIGH)
- 로직 복잡도 (복잡한 알고리즘 = HIGH)
- 의존성 (모듈 간 상호작용 = HIGH)
```
